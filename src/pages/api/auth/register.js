...
export async function POST({ request, cookies }) {
  try {
    ...
    if (code) {
      ...
      const card = await GiftCardService.getGiftCardByCode(cleanCode);
      ...
      // Ensure card status is active (paid)
      ...
      // Secure: get plan directly from database record
      finalPlan = card.plan;
    ...
    if (code) {
      const cleanCode = code.trim().toUpperCase();
      await GiftCardService.redeemGiftCard(cleanCode, newUser.id);
    }
